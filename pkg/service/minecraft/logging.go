...
func (s *loggingService) List(ctx context.Context, params model.ListParams) ([]*model.Minecraft, int64, error) {
	start := time.Now()
...
func (s *loggingService) Show(ctx context.Context, name string) (*model.Minecraft, error) {
	start := time.Now()
	...
	if err != nil && err != ErrNotFound {
		logger.Error().
			Err(err).
			Msg("Failed to find minecraft by name")
	} else {
		logger.Debug().
			Msg("")
	}
...
func (s *loggingService) Sync(ctx context.Context, versions version.Versions) error {
	...
	if err != nil {
		logger.Error().
			Err(err).
			Msg("Failed to sync minecraft versions")
	} else {
		logger.Debug().
			Msg("")
	}
...
func (s *loggingService) ListBuilds(ctx context.Context, params model.MinecraftBuildParams) ([]*model.Build, int64, error) {
	start := time.Now()
	...
	if err != nil {
		logger.Error().
			Err(err).
			Msg("Failed to find all minecraft builds")
	} else {
		logger.Debug().
			Msg("")
	}
...
func (s *loggingService) AttachBuild(ctx context.Context, params model.MinecraftBuildParams) error {
	start := time.Now()
...
func (s *loggingService) DropBuild(ctx context.Context, params model.MinecraftBuildParams) error {
	start := time.Now()
